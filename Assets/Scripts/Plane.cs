...
public class Plane : MonoBehaviour {
        ...
        get {
         ...
         set {
            ...
            if (health <= MaxHealth * .5f && health > 0) {
                ...
                damageEffect.GetComponentInChildren<ParticleSystem>().Play();
            } else {
    ...
    void Start() {
        ...
        player = GameObject.FindWithTag("PlayerPlane");
    }
    ...
    void Die() {
        ...
        damageEffect.GetComponentInChildren<ParticleSystem>().Pause();
        deathEffect.SetActive(true);
        deathEffect.GetComponentInChildren<ParticleSystem>().Play();
        if (!isPlayer)
        { 
            gameManager.score += 100;
            this.GetComponent<Target>().nameSuffix++;
        }
    }
    ...
    void UpdateThrottle(float dt) {
        ...
        jetEngine.volume = Throttle;
        jetEngine.pitch = Throttle;
    }
    ...
    void FixedUpdate() {
        ...
        {
            //this.target = player.GetComponent<Target>();
        }
    }
    ...
    void UpdateTarget()
    {
        ...
        GameObject midEnemy = null;
        GameObject farthestEnemy = null;

        float closestDistance = Mathf.Infinity;
        float farthestDistance = 0f;

        Vector3 playerPosition = this.transform.position; // Get the player's position
        GameObject[] enemyGOs = GameObject.FindGameObjectsWithTag("Enemy");

        // If there are no enemies, clear all targets and return
        ...
        Array.Sort(enemyGOs, (a, b) =>
            Vector3.Distance(playerPosition, a.transform.position).CompareTo(
                Vector3.Distance(playerPosition, b.transform.position)));

        // Assign closest, mid, and farthest enemies based on sorted array
        if (enemyGOs.Length >= 1)
        {
            closestEnemy = enemyGOs[0];
        }

        if (enemyGOs.Length >= 2)
        {
            midEnemy = enemyGOs[1];
        }

        if (enemyGOs.Length == 3)
        {
            farthestEnemy = enemyGOs[2];
        }

        // Assign targets based on availability
        this.target = closestEnemy != null ? closestEnemy.GetComponent<Target>() : null;
        this.midTarget = midEnemy;
        this.farTarget = farthestEnemy;
    }
